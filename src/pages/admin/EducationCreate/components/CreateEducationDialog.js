...
const CreateNewsDialog = ({ open, handleClose, refetch }) => {
    ...
    const [createNews] = educationApiSlice.useCreateEducationMutation();
    const [createImg] = educationApiSlice.useCreateImgMutation();
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (values.icon) {
                ...
                try {
                    const response = await createImg(formData).unwrap();
                    iconUrl = response.value; // or whatever field the image URL is in the response
                } catch (error) {
                    return;
                }
            }
